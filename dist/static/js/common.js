...
function getLocalTime(nS) {       //js把时间戳转为为普通日期格式
...
//无限下拉列表
function showAjax(currentIndex, pagesize, object) {
    object(currentIndex, pagesize);
}
function scrollFn(currentIndex, pagesize, object2, object, flagNoData) {
    //真实内容的高度
    ...
    if (flagNoData) { //数据全部加载完了
        ...
        object2(currentIndex, pagesize, object);
    }
};

//伪的下拉列表
    // function spuriousShow(isComplete, showpages){
    //   if(isComplete){
    //     showpages();
    //   }
    // }
